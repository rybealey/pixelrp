...
- :hit x throws a punch at whoever is selected. It lands only when the target stands on one of the four tiles sharing an EDGE with the attacker's - no diagonals, a tighter reach than :slap and :push, which allow the whole 3x3 block - and a landed punch costs the target 3-5 health. From any other tile the attacker swings and misses in public. Either outcome sets BOTH players' aggression to 100, which drains away over about 45 seconds, and costs a three second cooldown (shorter than :slap and the social commands, so a fight can trade blows). Fighting only works in unsafe zones, with one exception: two players who are both still aggressive can keep fighting inside a safe zone until their aggression runs out, so nobody escapes a fight by walking into one. Passive players (a Passive Smoothie, or City Government on duty) can neither hit nor be hit. A target reduced to 0 health is knocked out and cannot get up until staff :restore them. Health does not regenerate.
