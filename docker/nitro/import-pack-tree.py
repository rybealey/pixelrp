...
# Same renumber Bsstonino uses, and for the same reason: absolute positions
# assigned from the captions that are there land the same way however many
# times it runs, where nudging siblings down by one is correct exactly once.
EXTERIOR_EPILOGUE = """\
SET @n := 0;
UPDATE `catalog_pages` SET `order_num` = (@n := @n + 1)
    WHERE `parent_id` = @customs
...
# Asthorine brings together pieces 94 already sold on Medieval Doors (106095-
# 106111, all 17 of that page) with the new ones, so those catalog rows move to
# the new page and the emptied Medieval Doors is hidden rather than deleted -
# a hidden page is one flag to undo. Then the same renumber as Exterior Modern.
ASTHORINE_EPILOGUE = """\
UPDATE `catalog_items` SET `page_id` = 951000
    WHERE `page_id` = 944065 AND `item_id` IN (""" + ','.join("'%d'" % i for i in range(106095, 106112)) + """);
...
# Hangs off the designer import's Haaziq page rather than making a top-level
# category, because it is one creator's work. Resolved by id first and then by
# caption AND parent: 'Haaziq' is only unique inside Designer, and resolving a
# page on caption alone is how a migration ends up pointing at the wrong page.
CHEAPYXONY_PROLOGUE = """\
SET @builders := COALESCE(
    (SELECT `id` FROM `catalog_pages` WHERE `parent_id` = -1 AND `caption` = 'Builders' LIMIT 1),
    912362);
...
# Absolute positions from the captions that are there, so a re-run lands the
# same way - the same reason Bsstonino and Exterior Modern renumber.
CHEAPYXONY_EPILOGUE = """\
SET @n := 0;
UPDATE `catalog_pages` SET `order_num` = (@n := @n + 1)
...
# Corporations already exists (146 put the Corporation Gate there); Hospital
# hangs under it. Resolved by parent AND caption, never caption alone - the
# tab hubs reuse captions, and a bare caption match can land on the wrong one.
# Falling back to Builders rather than NULL keeps a DB that somehow lacks
# Corporations from aborting the deploy on a NOT NULL parent_id.
HOSPITAL_PROLOGUE = """\
SET @builders := COALESCE(
    (SELECT `id` FROM `catalog_pages` WHERE `parent_id` = -1 AND `caption` = 'Builders' LIMIT 1),
    912362);
...
PACKS = {
    'zara': {
        'root': '~/Desktop/Zara',
        'caption': 'Zara',
        'sql': repo('emulator/Resources/SQLs/Updates/166_ZaraFurni.sql'),
        'record_dir': repo('docker/nitro/zara-furni'),
        'furni_base': 110700,      # 165_AsthorineFurni ended at 110647
        'page_base': 952000,       # 951xxx is Asthorine's
        'category': 'customs',
        'furniline': 'customs',
        'prologue': EXTERIOR_PROLOGUE,
        'parent_sql': '@customs',
        'order_sql': '1',     # the epilogue's renumber is what actually places it
        'epilogue': EXTERIOR_EPILOGUE,
        'idempotence': "The sets under Customs are\n-- renumbered alphabetically at the end, which lands the same way every run.",
        'title': "the Zara set, in Builders > Customs > Zara",
        'blurb': """\
-- %(count)d clothing and rack pieces on ONE page under the existing Customs
...
-- ('Clothing 1', 'Rack 2').""",
    ...
    'asthorine': {
        'root': '~/Desktop/Asthorine',
        'caption': 'Asthorine',
        'sql': repo('emulator/Resources/SQLs/Updates/165_AsthorineFurni.sql'),
        'record_dir': repo('docker/nitro/asthorine-furni'),
        'furni_base': 110600,      # 161_PixelrpBin is 110500
        'page_base': 951000,       # 950xxx is Hospital's
        'category': 'customs',
        'furniline': 'customs',
        'prologue': EXTERIOR_PROLOGUE,
        'parent_sql': '@customs',
        'order_sql': '1',     # the epilogue's renumber is what actually places it
        'epilogue': ASTHORINE_EPILOGUE,
        'idempotence': "The 17 moved pieces are\n-- addressed by id, and the sets under Customs are renumbered alphabetically,\n-- so a re-run lands the same way.",
        'title': "the Asthorine set, in Builders > Customs > Asthorine",
        'blurb': """\
...
-- their catalog rows here, so the set reads as one page. The other %(count)d
-- are new.
--
...
-- ('Painting 11', 'Door 6 1x 2'); the 17 keep the names 94 gave them.""",
    ...
    'hospital': {
        'root': '~/Downloads/drive-download-20260921T103246Z-1-001',
        'caption': 'Hospital',
        'sql': repo('emulator/Resources/SQLs/Updates/147_HospitalFurni.sql'),
        'record_dir': repo('docker/nitro/hospital-furni'),
        'furni_base': 110400,      # 145_CheapyxonyFurni ended at 110308
        'page_base': 950000,       # 949xxx is the NFT/rares reorgs'
        'category': 'customs',
        'furniline': 'customs',
        'prologue': HOSPITAL_PROLOGUE,
        'parent_sql': '@corps',
        'order_sql': '1',
        'idempotence': "Hospital is the only page in its range.",
        'title': "the Modern Hospital set, in Builders > Corporations > Hospital",
        'blurb': """\
-- %(count)d pieces on ONE page under the existing Corporations category, beside
...
-- there is nothing in the download that says which one is the bed.""",
    ...
    'cheapyxony': {
        'root': '~/Downloads/Cheapyxony',
        'caption': 'Cheapyxony',
        'sql': repo('emulator/Resources/SQLs/Updates/145_CheapyxonyFurni.sql'),
        'record_dir': repo('docker/nitro/cheapyxony-furni'),
        'furni_base': 110000,      # 139_ExteriorModernFurni ended at 109932
        'page_base': 948000,       # 947xxx is Exterior Modern's, 949xxx the catalog reorgs'
        'category': 'designer',
        'furniline': 'designer',
        'prologue': CHEAPYXONY_PROLOGUE,
        'parent_sql': '@haaziq',
        'order_sql': '1',     # the epilogue's renumber is what actually places it
        'epilogue': CHEAPYXONY_EPILOGUE,
        'idempotence': "The pages under Haaziq are\n-- renumbered alphabetically at the end, which lands the same way every run.",
        'title': "Cheapyxony's furni, in Builders > Designer > Haaziq",
        'blurb': """\
-- %(count)d pieces hung under the EXISTING Haaziq designer page, beside the
...
-- inventing nicer ones would be inventing, not importing.""",
    ...
    'exterior': {
        'root': '~/Downloads/exterior modern',
        'caption': 'Exterior Modern',
        'sql': repo('emulator/Resources/SQLs/Updates/139_ExteriorModernFurni.sql'),
        'record_dir': repo('docker/nitro/exterior-modern-furni'),
        'furni_base': 109900,      # 122_BsstoninoFurni ended at 109899
        'page_base': 947000,       # 946xxx is the room-ad Images page
        'category': 'customs',
        'furniline': 'customs',
        'prologue': EXTERIOR_PROLOGUE,
        'parent_sql': '@customs',
        ...
        'epilogue': EXTERIOR_EPILOGUE,
        'idempotence': "The sets under Customs are\n-- renumbered alphabetically at the end, which lands the same way every run.",
        'title': "the Exterior Modern set, in Builders > Customs",
        'blurb': """\
-- %(count)d exterior pieces - blocks, fences, hedges, half walls, paving and
