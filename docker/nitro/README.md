...
- **Sunburnt faces (hd sets 6021 M / 6022 F) made free for everyone**
  (2026-08-08) — `sellable` flipped to false in FigureData.json so the sets
  show in the Change Your Looks picker without ownership; the emulator never
  gated them (no catalog_clothing rows). Regenerating FigureData from the
  converter restores sellable=true and re-hides them — re-apply after
  regeneration.
