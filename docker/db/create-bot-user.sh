...
# (bot/src/sso.ts), so it gets exactly the column privileges that statement
# needs and nothing else — full app credentials in the bot container would put
# the whole database in reach of a bot compromise.
#
...
SQL
  echo "[init] created bot MySQL user '$BOT_DB_USER' (SELECT(username), UPDATE(auth_ticket) on users)"
else
